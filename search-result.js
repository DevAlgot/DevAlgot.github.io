...
*/
    ...
    for (let i = 0; i < movies_results.results.length; i++) {
        ...
        if (!movies_results.results[i] || movies_results.results[i].media_type == "person" || movies_results.results[i].id == null) {
...
async function getDataNewQuery(movieName, page = 2) {
  var url = "https://api.themoviedb.org/3/search/multi?query=" + movieName + "&include_adult=false&language=en-US&page="+ page;
